...
export class GlobalvariablesService {
  ...
  private errorResponseCache = new Map<string, string>();
  private errorRequestInProgress = new Set<string>();

  private placeholderResponseCache = new Map<string, any>();
  private placeholderRequestInProgress = new Set<string>();

  private labelResponseCache = new Map<string, any>();
  private labelRequestInProgress = new Set<string>();
  menu_listJSON = [];
  ...
  private hasGlobalErrorOccurred = false;
  ...
  getMyCompErrors(formName) {
    if (this.previousErrorData && !this.previousErrorData.includes(formName)) {
      ...
      this.getErrors(formName).subscribe(async (res: any) => {
        ...
        const previousData = this.ErrorsMsgs.map(x => x.formCode);
  ...
  getLabels(formName): Observable<any> {
    const localeId = this.utilServ.langCode ? this.utilServ.langCode : 1;
    const cacheKey = `${formName}_${localeId}`;

    // ✅ Return from cache if available
    if (this.labelResponseCache.has(cacheKey)) {
      return of(this.labelResponseCache.get(cacheKey));
    }

    // ✅ Skip request if one is already in progress
    if (this.labelRequestInProgress.has(cacheKey)) {
      return of({}); // Return empty or default labels while loading
    }

    // ✅ Mark as in-progress
    this.labelRequestInProgress.add(cacheKey);

    return this.httpGet
      .getMasterList(`labelsWithForm?localeId=${localeId}&form=${formName}`)
      .pipe(
        tap((res: any) => {
          this.labelResponseCache.set(cacheKey, res);
          this.labelRequestInProgress.delete(cacheKey);
        }),
        catchError((err) => {
          this.labelRequestInProgress.delete(cacheKey);
          return of({}); // Return fallback if failed
        })
      );
  }

  getErrors(formName: string): Observable<any> {
    const localeId = this.utilServ.langCode ? this.utilServ.langCode : 1;

    //  If already cached, return it as Observable
    if (this.errorResponseCache.has(formName)) {
      return of({ response: this.errorResponseCache.get(formName) });
    }

    // If already fetching, don't re-fetch (return empty response or placeholder)
    if (this.errorRequestInProgress.has(formName)) {
      return of({ response: '' }); // or throwError or a Subject to notify when ready
    }

    //  Mark request as in progress
    this.errorRequestInProgress.add(formName);
    //  Make actual API call
    return this.httpGet.getMasterList(`gErrorsWithForm?localeId=${localeId}&form=${formName}`).pipe(
      tap((res: any) => {
        // ✅ Cache the response
        this.errorResponseCache.set(formName, res.response || '');
        // ✅ Remove from in-progress set
        this.errorRequestInProgress.delete(formName);
      }),
      catchError((err) => {
        this.errorRequestInProgress.delete(formName); // Ensure cleanup
        return of({ response: '' });
      })
    );
  }
  ...
  getPlaceholders(formName): Observable<any> {
    ...
    // ✅ If already cached, return the value as Observable
    if (this.placeholderResponseCache.has(formName)) {
      return of(this.placeholderResponseCache.get(formName));
    }

    // ✅ If request is already in progress, return a default/empty observable
    if (this.placeholderRequestInProgress.has(formName)) {
      return of({}); // Return empty object or default placeholder structure
    }

    // ✅ Mark as in progress
    this.placeholderRequestInProgress.add(formName);

    return this.httpGet
      .getMasterList(`placeholdersWithForm?localeId=${localeId}&form=${formName}`)
      .pipe(
        tap((res: any) => {
          this.placeholderResponseCache.set(formName, res); // cache response
          this.placeholderRequestInProgress.delete(formName);
        }),
        catchError((err) => {
          this.placeholderRequestInProgress.delete(formName);
          return of({}); // Return default empty result to continue stream
        })
      );
  }
  ...
  getGlobalErrors() {
    if (this.hasGlobalErrorOccurred) {
      return;
    }
    this.getErrors('global').subscribe((res: any) => {
      ...
      err => {
        this.hasGlobalErrorOccurred = true;
        console.error(err);
    });
  }
    ...
    this.httpGet.getMenuAccess().subscribe((data: any) => {
      ...
      if (row) {
        ...
        if (!rowa) {
          ...
          // this.router.navigate(['/effectivenes/Dashboard']);
